  ...
  //window.onload = function() {
    if (window.jQuery && $.fn.modal) {  
      ...
      */
      ...
      jquery = script('https://code.jquery.com/jquery-'+config.jquery+'.min.js'),
      bootstrap = {
        js : script('https://stackpath.bootstrapcdn.com/bootstrap/'+config.bootstrap+'/js/bootstrap.bundle.min.js'),
        css : stylesheet('https://stackpath.bootstrapcdn.com/bootstrap/'+config.bootstrap+'/css/bootstrap.min.css')
      };
      ...
      jquery.addEventListener ('load', () => {
        ...
        bootstrap.js.addEventListener ('load', () => {
          let element,index = 0;
          /**
           * 
           * @param {object} source 
           */
          function load(source) { // Load source
            if(typeof source === 'string') source = new Object({ type : 'js',url : source+'.js'})
            if(!source.type) source.type = source.url.split('.')[source.url.split('.').length];
            if(source.type === 'js') {
          ...
          if(typeof use.prototype === Array && typeof next ==='function') { 
            ...
            const source = use.map((item)=>Object({ url:item+'.js',type:'js'}));
            next = { use : source }
