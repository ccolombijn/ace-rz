 ...
 */
  ...
  */
  ...
  function btn(args){
    ...
    if(args.tooltip){
      ...
      /**  */
      const tooltipTitle = args.tooltip.title ? args.tooltip.title : args.tooltip;
      btn.setAttribute('title',tooltipTitle);
      const tooltipTrigger = args.tooltip.trigger ? {trigger : args.tooltip.trigger } : {trigger : 'hover focus'};
    ...
    if(typeof args.confirm === 'object'){ 
      ...
      $(btn).popover(args.confirm).on('shown.bs.popover', () => { // popover is shown
        ...
        const $confirmBtn = _('button',{class:`btn btn-sm btn-${args.confirm.class} confirm`},'Confirm')
            .on('click',confirm); // confirm method of confirm object in arguments
        $('.popover-body').append($confirmBtn);
        const $cancelBtn = _('button',{class:'btn btn-sm cancel'},'Cancel')
  ...
  */
  function navTabs(args) {
    const tabsElement = _('ul',{id:`tabs_${uid()}`,class:'nav nav-tabs','role':'tablist'}),
    ...
    tabs.forEach ((tab) => {
      if(tab.label){
        const tabId = uid(),
              ...
              tabLink = _('a',{ class : tabLinkClass, href : `#${tabId}`,'role':'tab','aria-controls':tabId,'data-toggle':'tab'},tab.label,['click',(event)=>{
    ...
    function calendarBuild(data){
      ...
      while(start < days ){
       ...
       // dd = (component.date('dd')/1);
        if(dd === (start/1)){
          $calendarTableRow.append($calendarTableCell.addClass('today').html(`<b>${start}</b>`));
        } else{
...
// component.queue
 ...
 function queue(args){
  ...
  if(typeof data.url === 'string'){
    api({url:data.url,callback:(response)=>{
    ...
    data.forEach((item)=>{
      if(item.url){
        api({url:item.url,callback:(response)=>{
    ...
    let output;
          ...
          if(item.component && item.options){
            ...
            if(item.element[1] && item.element[2] && item.element[3]){
              output = _(item.element[0],item.element[1],item.element[2],item.element[3]);
